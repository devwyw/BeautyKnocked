...
@implementation SusoensionFlowLayout
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    for (UICollectionViewLayoutAttributes *attributes in superArray) {
        ...
        if ([attributes.representedElementKind isEqualToString:UICollectionElementKindSectionHeader])
        {
            ...
            if (self.isAmendContentOffsetY) {
                CGFloat amendSetY = self.collectionView.contentOffset.y+_naviHeight > 25.0f ? self.collectionView.contentOffset.y - 25.0f : self.collectionView.contentOffset.y; //当滑动视图的调整已滚动高度后，为负数时，不需要给负数，直接给原本的值
                self.collectionView.contentOffset = CGPointMake(0.0f, amendSetY);
                self.isAmendContentOffsetY = NO;
            }
